...

{
    package Mock::Basic;
    use parent 'Teng';
    __PACKAGE__->load_plugin('SearchBySQLWithPager::MySQLFoundRows');
}
...
subtest 'simple' => sub {
    my ($rows, $pager) = $db->search_by_sql_with_pager(
        "SELECT SQL_CALC_FOUND_ROWS id, name FROM $table_name",
        [ ],
        $table_name,
        { rows => 3, page => 1 },
    );
    is join(',', map { $_->id } @$rows), '1,2,3';
    is $pager->total_entries(), 32, 'total_entries';
...
subtest 'last' => sub {
    my ($rows, $pager) = $db->search_by_sql_with_pager(
        "SELECT SQL_CALC_FOUND_ROWS id, name FROM $table_name WHERE id > ?",
        [ 0 ],
        $table_name,
        { rows => 3, page => 11 },
    );
    ...
    is $pager->previous_page, 10, 'previous_page';
};
